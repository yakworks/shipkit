...
config-bot-git-user: hasGitUser = $(shell git config --global user.email || true )
config-bot-git-user: | _verify_BOT_USER _verify_BOT_EMAIL
	@if [ ! "$(hasGitUser)" ] || [ "$(CI)" ]; then \
		echo "config_bot_user $(BOT_USER)"; \
